...
import '../../styles/optumYtdSuppressionStats.css'
      ...
      {items.map((item) => (
        <div key={item.label} className="ri-ytd-chart-legend-item" role="listitem">
          <span
            ...
            style={
              (item.kind === 'square' ? { backgroundColor: item.color } : { '--ri-legend-color': item.color }) as React.CSSProperties
            }
          />
              ...
              <div className="ri-ytd-impact-wrap">
                <div className="ri-ytd-impact-overlay">
                  {(completionChart.groups ?? []).map((group, index) => {
                    const layout: { start?: string; end?: string; bubble?: string } = completionChart.calloutLayouts?.[index] ?? {}
